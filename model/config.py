...
ones_then_zeroes = np.zeros(sim_length + 1)
for i in range(int(sim_length/2)):
    ones_then_zeroes[i] = 1
...
params = {
    ...
    # 'mean_discount_rate': [0.1, 0.5, 0.9, 0.99],  # this is the mean of the delegators' discount rates
    ...
    # gain function is an array of floats that is the multiplier for the actual dividend revenue per time period.
    # 'gain': [ones_then_zeroes],
    # 'gain': [square_wave(400, magnitude, sim_length) for magnitude in [2, 10]],
    'gain': [square_wave(400, magnitude, sim_length) for magnitude in [2]],
